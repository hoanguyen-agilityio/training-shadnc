import { IProductCard } from '@/types/product-card';

export const PRODUCTS_HOME_PAGE: (IProductCard & { value: string })[] = [
  // lorem1
  {
    img: 'https://picsum.photos/seed/1/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 1',
    brand: 'Al Karam',
    rating: 5,
    price: '$95.50',
    reviewLabel: '(4.1k) Customer Reviews',
    value: 'lorem1',
  },
  {
    img: 'https://picsum.photos/seed/2/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 2',
    brand: 'Al Karam',
    rating: 4,
    price: '$55.50',
    reviewLabel: '(3.2k) Customer Reviews',
    value: 'lorem1',
  },
  {
    img: 'https://picsum.photos/seed/3/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 3',
    brand: 'Al Karam',
    rating: 3,
    price: '$45.50',
    reviewLabel: '(1.1k) Customer Reviews',
    value: 'lorem1',
  },
  {
    img: 'https://picsum.photos/seed/4/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 4',
    brand: 'Al Karam',
    rating: 4,
    price: '$75.50',
    reviewLabel: '(2.9k) Customer Reviews',
    value: 'lorem1',
  },
  {
    img: 'https://picsum.photos/seed/5/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 5',
    brand: 'Al Karam',
    rating: 5,
    price: '$95.50',
    reviewLabel: '(4.8k) Customer Reviews',
    value: 'lorem1',
  },
  {
    img: 'https://picsum.photos/seed/6/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 6',
    brand: 'Al Karam',
    rating: 5,
    price: '$85.50',
    reviewLabel: '(3.5k) Customer Reviews',
    value: 'lorem1',
  },

  // lorem2
  {
    img: 'https://picsum.photos/seed/7/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 1',
    brand: 'Al Karam',
    rating: 5,
    price: '$65.50',
    reviewLabel: '(2.5k) Customer Reviews',
    value: 'lorem2',
  },
  {
    img: 'https://picsum.photos/seed/8/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 2',
    brand: 'Al Karam',
    rating: 4,
    price: '$55.50',
    reviewLabel: '(1.9k) Customer Reviews',
    value: 'lorem2',
  },
  {
    img: 'https://picsum.photos/seed/9/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 3',
    brand: 'Al Karam',
    rating: 3,
    price: '$45.50',
    reviewLabel: '(3.4k) Customer Reviews',
    value: 'lorem2',
  },
  {
    img: 'https://picsum.photos/seed/10/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 4',
    brand: 'Al Karam',
    rating: 4,
    price: '$75.50',
    reviewLabel: '(4.2k) Customer Reviews',
    value: 'lorem2',
  },
  {
    img: 'https://picsum.photos/seed/11/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 5',
    brand: 'Al Karam',
    rating: 5,
    price: '$95.50',
    reviewLabel: '(3.8k) Customer Reviews',
    value: 'lorem2',
  },
  {
    img: 'https://picsum.photos/seed/12/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 6',
    brand: 'Al Karam',
    rating: 5,
    price: '$85.50',
    reviewLabel: '(2.3k) Customer Reviews',
    value: 'lorem2',
  },

  // lorem3
  {
    img: 'https://picsum.photos/seed/13/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 1',
    brand: 'Al Karam',
    rating: 5,
    price: '$55.50',
    reviewLabel: '(1.9k) Customer Reviews',
    value: 'lorem3',
  },
  {
    img: 'https://picsum.photos/seed/14/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 2',
    brand: 'Al Karam',
    rating: 4,
    price: '$45.50',
    reviewLabel: '(3.4k) Customer Reviews',
    value: 'lorem3',
  },
  {
    img: 'https://picsum.photos/seed/15/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 3',
    brand: 'Al Karam',
    rating: 3,
    price: '$35.50',
    reviewLabel: '(2.2k) Customer Reviews',
    value: 'lorem3',
  },
  {
    img: 'https://picsum.photos/seed/16/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 4',
    brand: 'Al Karam',
    rating: 4,
    price: '$75.50',
    reviewLabel: '(3.2k) Customer Reviews',
    value: 'lorem3',
  },
  {
    img: 'https://picsum.photos/seed/17/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 5',
    brand: 'Al Karam',
    rating: 5,
    price: '$95.50',
    reviewLabel: '(4.8k) Customer Reviews',
    value: 'lorem3',
  },
  {
    img: 'https://picsum.photos/seed/18/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 6',
    brand: 'Al Karam',
    rating: 5,
    price: '$85.50',
    reviewLabel: '(3.5k) Customer Reviews',
    value: 'lorem3',
  },

  // lorem4
  {
    img: 'https://picsum.photos/seed/19/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 1',
    brand: 'Al Karam',
    rating: 5,
    price: '$65.50',
    reviewLabel: '(2.5k) Customer Reviews',
    value: 'lorem4',
  },
  {
    img: 'https://picsum.photos/seed/20/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 2',
    brand: 'Al Karam',
    rating: 4,
    price: '$55.50',
    reviewLabel: '(3.0k) Customer Reviews',
    value: 'lorem4',
  },
  {
    img: 'https://picsum.photos/seed/21/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 3',
    brand: 'Al Karam',
    rating: 3,
    price: '$45.50',
    reviewLabel: '(1.8k) Customer Reviews',
    value: 'lorem4',
  },
  {
    img: 'https://picsum.photos/seed/22/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 4',
    brand: 'Al Karam',
    rating: 4,
    price: '$75.50',
    reviewLabel: '(4.1k) Customer Reviews',
    value: 'lorem4',
  },
  {
    img: 'https://picsum.photos/seed/23/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 5',
    brand: 'Al Karam',
    rating: 5,
    price: '$95.50',
    reviewLabel: '(3.5k) Customer Reviews',
    value: 'lorem4',
  },
  {
    img: 'https://picsum.photos/seed/24/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 6',
    brand: 'Al Karam',
    rating: 5,
    price: '$85.50',
    reviewLabel: '(2.9k) Customer Reviews',
    value: 'lorem4',
  },

  // lorem5
  {
    img: 'https://picsum.photos/seed/25/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 1',
    brand: 'Al Karam',
    rating: 5,
    price: '$65.50',
    reviewLabel: '(2.5k) Customer Reviews',
    value: 'lorem5',
  },
  {
    img: 'https://picsum.photos/seed/26/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 2',
    brand: 'Al Karam',
    rating: 4,
    price: '$55.50',
    reviewLabel: '(1.9k) Customer Reviews',
    value: 'lorem5',
  },
  {
    img: 'https://picsum.photos/seed/27/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 3',
    brand: 'Al Karam',
    rating: 3,
    price: '$45.50',
    reviewLabel: '(2.4k) Customer Reviews',
    value: 'lorem5',
  },
  {
    img: 'https://picsum.photos/seed/28/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 4',
    brand: 'Al Karam',
    rating: 4,
    price: '$75.50',
    reviewLabel: '(3.9k) Customer Reviews',
    value: 'lorem5',
  },
  {
    img: 'https://picsum.photos/seed/29/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 5',
    brand: 'Al Karam',
    rating: 5,
    price: '$95.50',
    reviewLabel: '(4.5k) Customer Reviews',
    value: 'lorem5',
  },
  {
    img: 'https://picsum.photos/seed/30/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 6',
    brand: 'Al Karam',
    rating: 5,
    price: '$85.50',
    reviewLabel: '(3.7k) Customer Reviews',
    value: 'lorem5',
  },

  // lorem6
  {
    img: 'https://picsum.photos/seed/31/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 1',
    brand: 'Al Karam',
    rating: 5,
    price: '$65.50',
    reviewLabel: '(2.5k) Customer Reviews',
    value: 'lorem6',
  },
  {
    img: 'https://picsum.photos/seed/32/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 2',
    brand: 'Al Karam',
    rating: 4,
    price: '$55.50',
    reviewLabel: '(1.9k) Customer Reviews',
    value: 'lorem6',
  },
  {
    img: 'https://picsum.photos/seed/33/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 3',
    brand: 'Al Karam',
    rating: 3,
    price: '$45.50',
    reviewLabel: '(2.4k) Customer Reviews',
    value: 'lorem6',
  },
  {
    img: 'https://picsum.photos/seed/34/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 4',
    brand: 'Al Karam',
    rating: 4,
    price: '$75.50',
    reviewLabel: '(3.9k) Customer Reviews',
    value: 'lorem6',
  },
  {
    img: 'https://picsum.photos/seed/35/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 5',
    brand: 'Al Karam',
    rating: 5,
    price: '$95.50',
    reviewLabel: '(4.5k) Customer Reviews',
    value: 'lorem6',
  },
  {
    img: 'https://picsum.photos/seed/36/354/244',
    imgAlt: 'img product',
    variant: 'simple',
    title: 'Chinese cabbage 6',
    brand: 'Al Karam',
    rating: 5,
    price: '$85.50',
    reviewLabel: '(3.7k) Customer Reviews',
    value: 'lorem6',
  },
];
